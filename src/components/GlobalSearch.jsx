...
const GlobalSearch = () => {
    ...
    useEffect(() => {
        const performSearch = async () => {
            ...
            try {
                ...
                else if (searchType === 'flashcard') setResults(data.flashcardSets || []);
    ...
    const searchOptions = [
        { type: 'quiz', icon: <BrainCircuit size={18} />, label: t('globalSearch.quiz') },
        { type: 'flashcard', icon: <BookCopy size={18} />, label: t('globalSearch.flashcard') },
        ...
        <div className="relative w-full max-w-md" ref={searchContainerRef}>
            <div className="flex items-center bg-slate-800/80 backdrop-blur-sm rounded-full shadow-inner border border-slate-700">
                <div className="relative flex-grow flex items-center">
                    ...
                    <input
                        ...
                        placeholder={t('globalSearch.searchPlaceholder', { type: t(`globalSearch.${searchType}`) })}
                    ...
                    {searchOptions.map(opt => (
                        <button
                            ...
                            className={`p-1.5 rounded-full transition-colors duration-200 ${
                                searchType === opt.type ? 'bg-white/20 !text-white' : '!text-gray-400 hover:bg-white/10 hover:!text-white'
                            }`}
            ...
            {isFocused && searchTerm && (
                <div className="absolute top-full mt-2 w-full bg-slate-800 rounded-lg shadow-xl z-[50] overflow-hidden border border-slate-700">
                    {loading && <div className="p-4 text-center text-gray-400">{t('globalSearch.searching')}</div>}
                    ...
                    {!loading && results.length > 0 && (
                        <ul>
                            {results.map((item, index) => (
                                ...
                                >
                                    <p className="font-semibold text-gray-100">{item.title || item.username}</p>
                                    <p className="text-sm text-gray-400 line-clamp-1">{item.description || item.email}</p>
                                </li>
                            ))}
                        </ul>
                    )}
                    ...
                    {!loading && results.length === 0 && debouncedSearchTerm && (
                        <div className="p-4 text-center text-gray-400">{t('globalSearch.noResults')}</div>
                    )}
                </div>
            )}
        </div>
